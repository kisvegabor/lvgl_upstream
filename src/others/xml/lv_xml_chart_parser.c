...
void * lv_xml_chart_process(lv_xml_parser_state_t * state, const char ** attrs)
{
    ...
    for(int i = 0; attrs[i]; i += 2) {
        ...
        if(lv_streq("point_count", name)) lv_chart_set_point_count(item, atoi(value));
    }
    return item;
}
...
void * lv_xml_chart_series_process(lv_xml_parser_state_t * state, const char ** attrs)
{
    ...
    void * item = lv_chart_add_series(lv_xml_state_get_parent(state), lv_color_hex(strtol(color, NULL, 16)),
...
void * lv_xml_chart_cursor_process(lv_xml_parser_state_t * state, const char ** attrs)
{
    ...
    void * item = lv_chart_add_cursor(lv_xml_state_get_parent(state), lv_color_hex(strtol(color, NULL, 16)),
